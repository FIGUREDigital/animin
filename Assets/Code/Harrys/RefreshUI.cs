	...
	public void Start(){
		...
		for (int i=0; i<ProfilesManagementScript.Singleton.CurrentAnimin.Inventory.Count; ++i) {
			...
			if(InventoryItemData.Items[(int)ProfilesManagementScript.Singleton.CurrentAnimin.Inventory[i].Id].ItemType == PopupItemType.Food && !FoodIconSet){
				UIGlobalVariablesScript.Singleton.FoodButton.GetComponent<UIButton>().normalSprite = InventoryItemData.Items[(int)ProfilesManagementScript.Singleton.CurrentAnimin.Inventory[i].Id].SpriteName;
				...
				UIGlobalVariablesScript.Singleton.ItemsButton.GetComponent<UIButton>().normalSprite = InventoryItemData.Items[(int)ProfilesManagementScript.Singleton.CurrentAnimin.Inventory[i].Id].SpriteName;
				...
				UIGlobalVariablesScript.Singleton.MedicineButton.GetComponent<UIButton>().normalSprite = InventoryItemData.Items[(int)ProfilesManagementScript.Singleton.CurrentAnimin.Inventory[i].Id].SpriteName;
