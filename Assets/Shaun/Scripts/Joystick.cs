...
//  private float               firstDeltaTime          = .5F;
	...
	public void Start () {
		...
		if (touchPad) {
			...
			guiTouchOffset.x = defaultRect.width * 0.2F;                                                    //This is an offset for touch input to match with the top left corner of GUI
			guiTouchOffset.y = defaultRect.height * 0.2F;
	...
	public void Update () {
			...
			for (int i = 0; i < count; i++) {
				...
				if (lastFingerId == fingerID) {
					if (Application.platform == RuntimePlatform.IPhonePlayer || Application.platform == RuntimePlatform.Android) { //If we are are on the actual device
						touch = Input.GetTouch (i);
						if (touch.tapCount > tapCount) {                                                        // Override the tap count with what the iPhone SDK reports if it is greater                                                                                       // This is a workaround, since the iPhone SDK does not currently track taps
		...
		Globals.joystickPosition = position;
		//Debug.Log ("Globals.joystickPosition	:	"	+	Globals.joystickPosition);
		
		if (position.x != 0 && position.y != 0) {
			Globals.dragDistance = Vector2.Distance(position, Vector2.zero);
			if (Globals.dragDistance > 1.0F) Globals.dragDistance = 1.0F;
		} else {
			Globals.dragDistance = 0.0F;
		}
		//Debug.Log ("Globals.dragDistance		:	"	+	Globals.dragDistance);
	}
